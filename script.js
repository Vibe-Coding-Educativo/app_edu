document.addEventListener('DOMContentLoaded', () => {
    ...
    function processData(data) {
        ...
        // ---------- BLOQUE ACTUALIZADO ----------
        const deleteCutoff = new Map(); // url → timestamp (ms) de la marca más reciente
    
        mappedData.forEach(app => {
            if (normalizeString(app.eliminar_registro) === 'si' && app.url_app) {
                const url = app.url_app.trim();
                const ts = new Date(app.timestamp).getTime(); // convierte la fecha
                const prev = deleteCutoff.get(url) ?? -Infinity; // nos quedamos con la MÁS NUEVA
        ...
        const appsWithoutDeleted = mappedData.filter(app => {
            ...
            if (cut === undefined) return true; // nadie la ha marcado
            return new Date(app.timestamp).getTime() > cut; // conserva las posteriores
        });
        // ----------------------------------
