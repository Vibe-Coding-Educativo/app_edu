document.addEventListener('DOMContentLoaded', () => {
    ...
    let activeFavoriteTab = 'General'; // Para saber qué pestaña de favoritos está activa
    ...
    const elements = {
        ...
        favoritesTabsContainer: document.getElementById('favorites-tabs-container'), // Nuevo
    ...
    // INICIO: Nueva lógica para renderizar pestañas y su contenido
    function renderFavoriteTabsAndContent(allFavoriteApps) {
        ...
        sortedCategories.forEach(category => {
            ...
            tabButton.textContent = `${category} (${favorites[category].length})`;
        ...
        elements.appsContainer.className = ''; // Reset grid classes from main container
    }
    // FIN: Nueva lógica para renderizar pestañas
    ...
    function createAppCard(app) {
        ...
        card.innerHTML = `
            <div class="p-5 flex-grow flex flex-col relative">
                <button class="favorite-btn absolute top-3 right-3 p-1 rounded-full bg-white/50 hover:bg-white/90 focus:outline-none focus:ring-2 focus:ring-red-400" title="Gestionar favorito" data-key="${app.key}">
    ...
    // --- Lógica de filtros, URL y eventos ---

    const paramMapping = { subject: 'area_conocimiento', level: 'nivel_educativo', type: 'tipo_recurso', platform: 'plataforma', author: 'nombre_autor', keyword: 'palabras_clave', search: 'search', fav_category: 'fav_category' };

    // INICIO: Modificada para generar URL de categoría
    function generateShareableURL(type = 'filters', value = '') {
        const params = new URLSearchParams();
        if (type === 'category' && value && favorites[value]) {
            params.set('fav_category', value);
            params.set('ids', favorites[value].join(','));
        } else {
    ...
    // FIN: Modificada para generar URL de categoría

    // INICIO: Modificada para gestionar la vista de categoría compartida
    function applyUrlParams() {
        ...
        if (ids) {
            ...
            const message = favCategory
                ? `Estás viendo la categoría de favoritos "<strong>${favCategory}</strong>".`
                : `Estás viendo una colección personalizada de <strong>${sharedApps.length}</strong> aplicaciones.`;
            
            elements.customViewMsg.innerHTML = `${message} <button id="exit-custom-view" class="font-bold underline ml-2 hover:text-blue-600">Ver todas</button>`;
            ...
            elements.resultsCounter.textContent = `Mostrando una colección de ${sharedApps.length} aplicaciones.`;
        } else {
            isCustomView = false;
            applyFiltersFromURL();
            applyAndDisplay();
    ...
    // FIN: Modificada para vista de categoría
    ...
    // El resto del JS (lógica de inicialización, eventos, modales, etc.)
    // es prácticamente igual al de la versión anterior.
    // ... (Se omite por brevedad, es el mismo código que ya tenías)
    // Solo se pegan las funciones que han sido modificadas o son relevantes
    // para el cambio solicitado.
    ...
    function setupEventListeners() {
        ...
        elements.toggleFavoritesBtn.addEventListener('click', () => {
            ...
            currentPage = 1;
            if (showingFavoritesOnly) {
                const favCats = Object.keys(favorites);
                activeFavoriteTab = favCats.includes(activeFavoriteTab) ? activeFavoriteTab : 'General';
                elements.filterPanel.classList.add('hidden');
        ...
        elements.appsContainer.addEventListener('click', e => {
            ...
            if (keywordTag) { e.preventDefault(); const keyword = keywordTag.dataset.keyword; currentPage = 1; if (activeFilters.palabras_clave.has(keyword)) { activeFilters.palabras_clave.delete(keyword); } else { activeFilters.palabras_clave.add(keyword); } applyAndDisplay(); return; }
            const filterLink = e.target.closest('.filter-link');
            if (filterLink) { e.preventDefault(); const { category, filter } = filterLink.dataset; const correspondingButton = document.querySelector(`#filters-container [data-category='${category}'] [data-filter='${filter}']`); if (correspondingButton) { currentPage = 1; correspondingButton.click(); } }
            const target = e.target;
            if (target.classList.contains('read-more') || target.classList.contains('read-less')) { e.preventDefault(); const container = target.closest('.description-container'); container.querySelector('.short-desc').classList.toggle('hidden'); container.querySelector('.long-desc').classList.toggle('hidden'); }
        });

        // Event listeners para los modales, botones de limpiar, etc.
    ...
    function clearAllFavorites() { if (getTotalFavoritesCount() > 0) { currentPage = 1; favorites = { "General": [] }; activeFavoriteTab = 'General'; saveFavorites(); applyAndDisplay(); } }
