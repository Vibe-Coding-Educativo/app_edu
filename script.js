document.addEventListener('DOMContentLoaded', () => {
    ...
    function getAuthorAccentColor(authorName) {
        const normalizedAuthor = normalizeString(authorName || '');
        if (!normalizedAuthor) return 'hsl(215 20% 68%)';

        let hash = 0;
        for (let i = 0; i < normalizedAuthor.length; i++) {
            hash = ((hash << 5) - hash) + normalizedAuthor.charCodeAt(i);
            hash |= 0;
        }

        const hue = Math.abs(hash) % 360;
        const saturation = 62;
        const lightness = (hue >= 40 && hue <= 75) ? 48 : 56;
